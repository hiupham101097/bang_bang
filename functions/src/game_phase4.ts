...
const cardType = (id: string) =>
  specialTypes.find((type) => id === type || id.startsWith(`${type}_`));
...
export const playSpecialCard = onCall(async (request) => {
  ...
  const { roomId, cardId, targetPlayerId } = request.data as {
    roomId?: string;
    cardId?: string;
    targetPlayerId?: string;
  };
  if (!roomId || !cardId)
    throw new HttpsError("invalid-argument", "Thiếu roomId hoặc cardId.");
  const type = cardType(cardId);
  if (!type)
    throw new HttpsError(
      "failed-precondition",
      "Thẻ này chưa thuộc nhóm hiệu ứng đơn.",
    );
  ...
  await db.runTransaction(async (tx) => {
    ...
    const roomUpdate: Record<string, unknown> = {
      ...
      publicLog: appendPublicLog(
        state,
        `${["barrel", "mustang", "appaloosa", "volcanic", "dynamite", "jail"].includes(type) ? "equip" : "play"}:${uid}:${cardId}${targetPlayerId ? `:${targetPlayerId}` : ""}`,
      ),
      updatedAt: serverNow(),
    };
    if (type === "dilizenza" || type === "wells_fargo") {
      ...
      if (pile.length < drawCount)
        throw new HttpsError("failed-precondition", "Không đủ bài để rút.");
      ...
      if (!targetPlayerId || targetPlayerId === state.get("sheriffPlayerId"))
        throw new HttpsError(
          "failed-precondition",
          "Không thể đặt Jail lên Cảnh sát trưởng.",
        );
      const target = await tx.get(
        room.collection("players").doc(targetPlayerId),
      );
      if (!target.exists || target.get("isAlive") === false)
        throw new HttpsError("failed-precondition", "Mục tiêu không hợp lệ.");
      ...
      if (type === "dynamite" && actor.get(field))
        throw new HttpsError("failed-precondition", "Bạn đã có Dynamite.");
      const old = actor.get(field);
      if (old) discard.push(old);
      const extra =
        type === "mustang"
          ? { distanceFromOthersModifier: 1 }
          : type === "appaloosa"
            ? { distanceToOthersModifier: -1 }
            : type === "volcanic"
              ? { attackRange: 1, unlimitedBang: false }
              : {};
      tx.update(actor.ref, {
        ...extra,
        [field]: cardId,
...
export const resolveTurnJudgments = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      ...
      throw new HttpsError("failed-precondition", "Chưa đến bước phán xét.");
    const pile = [...(deck.get("drawPile") ?? [])] as string[];
    const discard = [...(deck.get("discardPile") ?? [])] as string[];
    if (pile.length === 0)
      throw new HttpsError("failed-precondition", "Hết bài để phán xét.");
    const players = (await tx.get(room.collection("players"))).docs;
    const judgmentLogs: string[] = [];
    const dynamite = player.get("dynamiteCardId") as string | undefined;
    if (dynamite) {
      const judgement = pile.shift()!;
      discard.push(judgement);
      const explodes =
        suitOf(judgement) === "spade" &&
        [
          "two",
          "three",
          "four",
      ...
      judgmentLogs.push(
        `judgment:${uid}:${judgement}:dynamite:${explodes ? "explode" : "pass"}`,
      );
      if (explodes) {
        ...
        tx.update(room, {
          ...
          dyingPlayerId:
            health <= 0 ? uid : admin.firestore.FieldValue.delete(),
          deckRemainingCount: pile.length,
          discardCount: discard.length,
...
export const resolveTargetCard = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, cardId, targetPlayerId, equipmentCardId } = request.data as {
    roomId?: string;
    cardId?: string;
    targetPlayerId?: string;
  ...
  if (!roomId || !cardId || !targetPlayerId || targetPlayerId === uid)
    throw new HttpsError("invalid-argument", "Dữ liệu mục tiêu không hợp lệ.");
  const type = cardId.startsWith("panico_")
    ? "panico"
  ...
  if (!type)
    throw new HttpsError(
      "failed-precondition",
      "Không phải Panic! hoặc Cat Balou.",
    );
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    ...
    const targetState = await tx.get(
      room.collection("privateStates").doc(targetPlayerId),
    );
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    if (
      !state.exists ||
    ...
    if (type === "panico") {
      const alive = (await tx.get(room.collection("players"))).docs
        .filter((player) => player.get("isAlive") !== false)
        .sort((a, b) => Number(a.get("seat")) - Number(b.get("seat")));
      const actorIndex = alive.findIndex((player) => player.id === uid);
      const targetIndex = alive.findIndex(
        (player) => player.id === targetPlayerId,
      );
      const baseDistance = Math.min(
        (targetIndex - actorIndex + alive.length) % alive.length,
        (actorIndex - targetIndex + alive.length) % alive.length,
      );
      ...
      if (distance > 1)
        throw new HttpsError(
          "failed-precondition",
          "Mục tiêu ngoài tầm Panico.",
        );
    }
    const hand = [...(actorState.get("handCardIds") ?? [])] as string[];
    const cardIndex = hand.indexOf(cardId);
    if (cardIndex < 0)
      throw new HttpsError("permission-denied", "Bạn không có lá bài này.");
    const targetHand = [...(targetState.get("handCardIds") ?? [])] as string[];
    let field: string | undefined;
    let taken: string | undefined;
    if (equipmentCardId) {
      field = publicEquipmentFields.find(
        (name) => target.get(name) === equipmentCardId,
      );
      if (!field)
        throw new HttpsError(
          "failed-precondition",
          "Trang bị mục tiêu không hợp lệ.",
        );
    ...
    if (!taken)
      throw new HttpsError(
        "failed-precondition",
        "Mục tiêu không còn bài hoặc trang bị.",
      );
    const nextHand = [
    ...
    const discard = [...(deck.get("discardPile") ?? []), cardId] as string[];
    if (type === "panico") {
    ...
    else tx.update(targetState.ref, { handCardIds: nextTargetHand });
    tx.update(target.ref, {
      cardCount: field ? target.get("cardCount") : nextTargetHand.length,
    });
    tx.update(deck.ref, { discardPile: discard });
    tx.update(room, {
      cardsPlayedThisTurn: 1,
      discardCount: discard.length,
      discardTopCardId: discard.at(-1),
      publicLog: appendPublicLog(
        state,
        `play:${uid}:${cardId}:${targetPlayerId}`,
      ),
      updatedAt: serverNow(),
    });
  });
...
export const openGeneralStore = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, cardId, actionId } = request.data as {
    roomId?: string;
    cardId?: string;
    actionId?: string;
  };
  if (!roomId || !cardId || !actionId || !cardId.startsWith("general_store_"))
    throw new HttpsError(
      "invalid-argument",
      "Dữ liệu General Store không hợp lệ.",
    );
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    ...
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    const action = await tx.get(
      room.collection("pendingActions").doc(actionId),
    );
    if (
      !state.exists ||
      state.get("phase") !== "play_phase" ||
    ...
    const order = [
      ...players.slice(actorIndex),
      ...players.slice(0, actorIndex),
    ].map((player) => player.id);
    const pile = [...(deck.get("drawPile") ?? [])] as string[];
    if (pile.length < order.length)
      throw new HttpsError(
        "failed-precondition",
        "Không đủ bài để mở cửa hàng.",
      );
    ...
    tx.update(room, {
      phase: "waiting_general_store_selection",
      cardsPlayedThisTurn: 1,
      deckRemainingCount: pile.length,
      discardCount: discard.length,
      discardTopCardId: cardId,
...
export const chooseGeneralStoreCard = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, actionId, cardId } = request.data as {
    roomId?: string;
    actionId?: string;
    cardId?: string;
  };
  if (!roomId || !actionId || !cardId)
    throw new HttpsError("invalid-argument", "Thiếu dữ liệu chọn bài.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const action = await tx.get(
    ...
    if (
      !state.exists ||
      state.get("phase") !== "waiting_general_store_selection" ||
      !action.exists ||
      action.get("status") !== "waiting" ||
      action.get("currentPickerId") !== uid
    )
      throw new HttpsError("failed-precondition", "Chưa đến lượt chọn bài.");
    const opened = [...(action.get("openedCardIds") ?? [])] as string[];
    if (!opened.includes(cardId))
      throw new HttpsError("failed-precondition", "Lá bài không còn trên bàn.");
...
export const startMultiAttack = onCall(async (request) => {
  ...
  const type = cardId?.startsWith("gatling_")
    ? "gatling"
    : cardId?.startsWith("indiani_")
      ? "indiani"
      : null;
  if (!roomId || !cardId || !actionId || !type)
    throw new HttpsError(
      "invalid-argument",
      "Thẻ tấn công nhiều mục tiêu không hợp lệ.",
    );
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    ...
    const index = hand.indexOf(cardId);
    if (index < 0)
      throw new HttpsError("permission-denied", "Bạn không có lá này.");
    const targets = (await tx.get(room.collection("players"))).docs
      .filter((player) => player.id !== uid && player.get("isAlive") !== false)
      .sort((a, b) => Number(a.get("seat")) - Number(b.get("seat")))
      .map((player) => player.id);
    const nextHand = [...hand.slice(0, index), ...hand.slice(index + 1)];
    const discard = [...(deck.get("discardPile") ?? []), cardId];
...
export const respondMultiAttack = onCall(async (request) => {
  ...
  if (!roomId || !actionId)
    throw new HttpsError("invalid-argument", "Thiếu actionId.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const action = await tx.get(
    ...
    if (
      !state.exists ||
      state.get("phase") !== "waiting_multi_response" ||
      !action.exists ||
      action.get("currentTargetId") !== uid ||
      !player.exists
    )
      throw new HttpsError("failed-precondition", "Chưa đến lượt phản ứng.");
    const actionType = action.get("actionType");
    const needed = actionType === "gatling" ? "dodge" : "bang";
    const hand = [...(privateState.get("handCardIds") ?? [])] as string[];
    const valid =
      cardId && (cardId === needed || cardId.startsWith(`${needed}_`));
    let nextHand = hand;
    const discard = [...(deck.get("discardPile") ?? [])] as string[];
...
export const startDuel = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, cardId, targetPlayerId, actionId } = request.data as {
    roomId?: string;
    cardId?: string;
    targetPlayerId?: string;
    actionId?: string;
  ...
  if (
    ...
    throw new HttpsError("invalid-argument", "Dữ liệu Duel không hợp lệ.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const actor = await tx.get(room.collection("players").doc(uid));
    ...
    if (
      ...
      throw new HttpsError(
        "failed-precondition",
        "Không thể đấu súng lúc này.",
      );
    const hand = [...(privateState.get("handCardIds") ?? [])] as string[];
    const index = hand.indexOf(cardId);
    if (index < 0)
      throw new HttpsError("permission-denied", "Bạn không có lá Duel.");
    const nextHand = [...hand.slice(0, index), ...hand.slice(index + 1)];
    const discard = [...(deck.get("discardPile") ?? []), cardId];
    tx.update(privateState.ref, { handCardIds: nextHand });
    tx.update(actor.ref, { cardCount: nextHand.length });
...
export const respondDuel = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      !state.exists ||
      state.get("phase") !== "waiting_duel_response" ||
      !action.exists ||
      action.get("currentResponderId") !== uid ||
      !player.exists
    )
      throw new HttpsError("failed-precondition", "Chưa đến lượt đáp Duel.");
    const hand = [...(privateState.get("handCardIds") ?? [])] as string[];
    const validBang =
      cardId &&
      (cardId === "bang" || cardId.startsWith("bang_")) &&
      hand.includes(cardId);
    if (validBang) {
      const index = hand.indexOf(cardId!);
      const nextHand = [...hand.slice(0, index), ...hand.slice(index + 1)];
      const other =
        action.get("actorPlayerId") === uid
...
export const useSidKetchum = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, cardIds } = request.data as {
    roomId?: string;
    cardIds?: string[];
  };
  if (
    ...
    throw new HttpsError(
      "invalid-argument",
      "Sid Ketchum cần bỏ đúng 2 lá khác nhau.",
    );
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    ...
    if (
      !state.exists ||
      state.get("status") !== "playing" ||
      player.get("characterId") !== "sid_ketchum" ||
      player.get("isAlive") === false ||
      Number(player.get("health")) >= Number(player.get("maxHealth"))
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể dùng kỹ năng Sid Ketchum.",
      );
    const hand = [...(privateState.get("handCardIds") ?? [])] as string[];
    if (!cardIds.every((card) => hand.includes(card)))
      throw new HttpsError(
        "permission-denied",
        "Có lá bài không thuộc tay bạn.",
      );
    ...
    tx.update(player.ref, {
      health: Math.min(
        Number(player.get("maxHealth")),
        Number(player.get("health")) + 1,
      ),
      cardCount: nextHand.length,
    });
    tx.update(deck.ref, { discardPile: discard });
...
export const drawCharacterTurnCards = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, actionId } = request.data as {
    roomId?: string;
    actionId?: string;
  };
  if (!roomId || !actionId)
    throw new HttpsError("invalid-argument", "Thiếu roomId hoặc actionId.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const player = await tx.get(room.collection("players").doc(uid));
    ...
    if (
      ...
      throw new HttpsError("failed-precondition", "Không thể rút bài lúc này.");
    const pile = [...(deck.get("drawPile") ?? [])] as string[];
    const discard = [...(deck.get("discardPile") ?? [])] as string[];
...
export const drawJesseJones = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, targetPlayerId, actionId } = request.data as {
    roomId?: string;
    targetPlayerId?: string;
    actionId?: string;
  };
  if (!roomId || !actionId)
    throw new HttpsError("invalid-argument", "Thiếu dữ liệu rút bài.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const player = await tx.get(room.collection("players").doc(uid));
    ...
    if (
      ...
      Number(state.get("judgmentsResolvedForTurn") ?? 0) !==
        Number(state.get("turnNumber") ?? 0) ||
      player.get("characterId") !== "jesse_jones" ||
      action.exists
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể dùng Jesse Jones lúc này.",
      );
    const pile = [...(deck.get("drawPile") ?? [])] as string[];
    if (!pile.length) throw new HttpsError("failed-precondition", "Hết bài.");
    const drawn: string[] = [];
    if (targetPlayerId && targetPlayerId !== uid) {
      ...
      if (
        ...
        throw new HttpsError("failed-precondition", "Mục tiêu không có bài.");
    ...
    while (drawn.length < 2) {
      if (!pile.length)
        throw new HttpsError("failed-precondition", "Không đủ bài.");
      drawn.push(pile.shift()!);
    }
...
export const openKitCarlson = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      ...
      Number(state.get("judgmentsResolvedForTurn") ?? 0) !==
        Number(state.get("turnNumber") ?? 0) ||
      player.get("characterId") !== "kit_carlson" ||
      action.exists ||
      pile.length < 3
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể mở kỹ năng Kit Carlson.",
      );
    const choices = pile.splice(0, 3);
    tx.update(deck.ref, { drawPile: pile });
    tx.set(action.ref, {
      actionId,
    ...
    tx.update(room, {
      phase: "character_skill_selection",
      deckRemainingCount: pile.length,
      updatedAt: serverNow(),
    });
  });
...
export const chooseKitCarlson = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, actionId, cardIds } = request.data as {
    roomId?: string;
    actionId?: string;
    cardIds?: string[];
  };
  if (
    ...
    throw new HttpsError("invalid-argument", "Cần chọn đúng 2 lá.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const action = await tx.get(
    ...
    const own = await tx.get(room.collection("privateStates").doc(uid));
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    const choices = [...(action.get("choices") ?? [])] as string[];
    if (
      !state.exists ||
      state.get("phase") !== "character_skill_selection" ||
      !action.exists ||
      action.get("actorPlayerId") !== uid ||
      !cardIds.every((id) => choices.includes(id))
    )
      throw new HttpsError(
        "failed-precondition",
        "Lựa chọn Kit Carlson không hợp lệ.",
      );
    const remain = choices.filter((id) => !cardIds.includes(id));
    const pile = [...remain, ...(deck.get("drawPile") ?? [])];
    const hand = [...(own.get("handCardIds") ?? []), ...cardIds];
    tx.update(own.ref, { handCardIds: hand });
    tx.update(player.ref, { cardCount: hand.length });
    tx.update(deck.ref, { drawPile: pile });
    tx.delete(action.ref);
...
export const openLuckyDukeJudgment = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, actionId, judgmentContext } = request.data as {
    roomId?: string;
    actionId?: string;
    judgmentContext?: string;
  };
  if (!roomId || !actionId || !judgmentContext)
    throw new HttpsError("invalid-argument", "Thiếu dữ liệu phán xét.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const player = await tx.get(room.collection("players").doc(uid));
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    ...
    if (
      !player.exists ||
      player.get("characterId") !== "lucky_duke" ||
      action.exists ||
      pile.length < 2
    )
      throw new HttpsError("failed-precondition", "Không thể dùng Lucky Duke.");
    const choices = [pile.shift()!, pile.shift()!];
    tx.update(deck.ref, { drawPile: pile });
    tx.set(action.ref, {
      actionId,
...
export const chooseLuckyDukeJudgment = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, actionId, resultCardId } = request.data as {
    roomId?: string;
    actionId?: string;
    resultCardId?: string;
  };
  if (!roomId || !actionId || !resultCardId)
    throw new HttpsError("invalid-argument", "Thiếu kết quả phán xét.");
  const room = db.doc(`rooms/${roomId}`);
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      !action.exists ||
      action.get("actionType") !== "lucky_duke_judgment" ||
      action.get("actorPlayerId") !== uid
    )
      throw new HttpsError(
        "failed-precondition",
        "Phán xét Lucky Duke không hợp lệ.",
      );
    const choices = [...(action.get("choices") ?? [])] as string[];
    if (!choices.includes(resultCardId))
      throw new HttpsError(
        "invalid-argument",
        "Kết quả không thuộc hai lá đã rút.",
      );
    result = resultCardId;
    tx.update(deck.ref, {
      discardPile: [...(deck.get("discardPile") ?? []), ...choices],
    });
    tx.delete(action.ref);
  });
...
export const resolveJourdonnais = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, pendingActionId } = request.data as {
    roomId?: string;
    pendingActionId?: string;
  };
  if (!roomId || !pendingActionId)
    throw new HttpsError("invalid-argument", "Thiếu dữ liệu phản ứng.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const pending = await tx.get(
      room.collection("pendingActions").doc(pendingActionId),
    );
    const player = await tx.get(room.collection("players").doc(uid));
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    if (
      !state.exists ||
      state.get("phase") !== "waiting_response" ||
      !pending.exists ||
      pending.get("actionType") !== "bang" ||
      pending.get("targetPlayerId") !== uid ||
      player.get("characterId") !== "jourdonnais" ||
      Number(pending.get("requiredDodges") ?? 1) > 1
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể phán xét Jourdonnais.",
      );
    const pile = [...(deck.get("drawPile") ?? [])] as string[];
    if (!pile.length)
      throw new HttpsError("failed-precondition", "Hết bài phán xét.");
    const judgment = pile.shift()!;
    const discard = [...(deck.get("discardPile") ?? []), judgment];
    tx.update(deck.ref, { drawPile: pile, discardPile: discard });
    if (suitOf(judgment) === "heart") {
      ...
      tx.update(room, {
        ...
        deckRemainingCount: pile.length,
        discardCount: discard.length,
        discardTopCardId: judgment,
        updatedAt: serverNow(),
      });
    } else
      tx.update(pending.ref, {
        jourdonnaisJudgmentCardId: judgment,
        judgmentFailed: true,
      });
  });
  return {};
...
export const useCalamityJanetDodge = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, pendingActionId, cardId } = request.data as {
    roomId?: string;
    pendingActionId?: string;
    cardId?: string;
  };
  if (!roomId || !pendingActionId || !cardId?.startsWith("bang_"))
    throw new HttpsError("invalid-argument", "Cần một lá Bang.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const pending = await tx.get(
    ...
    if (
      ...
      !pending.exists ||
      pending.get("targetPlayerId") !== uid ||
      player.get("characterId") !== "calamity_janet" ||
      Number(pending.get("requiredDodges") ?? 1) > 1 ||
      !hand.includes(cardId)
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể dùng kỹ năng Calamity Janet.",
      );
    const nextHand = hand.filter((card) => card !== cardId);
    const discard = [...(deck.get("discardPile") ?? []), cardId];
    tx.update(privateState.ref, { handCardIds: nextHand });
    tx.update(player.ref, { cardCount: nextHand.length });
    tx.update(deck.ref, { discardPile: discard });
...
export const resolveSlabDodge = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const uid = request.auth.uid;
  const { roomId, pendingActionId, cardIds } = request.data as {
    roomId?: string;
    pendingActionId?: string;
    cardIds?: string[];
  };
  if (
    ...
    throw new HttpsError("invalid-argument", "Cần đúng 2 lá Né.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const pending = await tx.get(
    ...
    const own = await tx.get(room.collection("privateStates").doc(uid));
    const deck = await tx.get(room.collection("serverState").doc("deck"));
    const attacker = await tx.get(
      room
        .collection("players")
        .doc(String(pending.get("actorPlayerId") ?? "")),
    );
    const hand = [...(own.get("handCardIds") ?? [])] as string[];
    if (
      !pending.exists ||
      pending.get("actionType") !== "bang" ||
      pending.get("targetPlayerId") !== uid ||
      !attacker.exists ||
      attacker.get("characterId") !== "slab_the_killer" ||
      !cardIds.every((card) => hand.includes(card) && card.startsWith("dodge_"))
    )
      throw new HttpsError(
        "failed-precondition",
        "Không thể né Bang của Slab.",
      );
    const nextHand = hand.filter((card) => !cardIds.includes(card));
    const discard = [...(deck.get("discardPile") ?? []), ...cardIds];
    tx.update(own.ref, { handCardIds: nextHand });
    tx.update(target.ref, { cardCount: nextHand.length });
    tx.update(deck.ref, { discardPile: discard });
    tx.delete(pending.ref);
    tx.update(room, {
      phase: "play_phase",
...
export const acceptBangDamage = onCall(async (request) => {
  ...
  if (!roomId || !pendingActionId)
    throw new HttpsError("invalid-argument", "Thiếu action sát thương.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const state = await tx.get(room);
    const pending = await tx.get(
    ...
    if (
      ...
      throw new HttpsError(
        "failed-precondition",
        "Không có Bang cần nhận sát thương.",
      );
    let hand = [...(own.get("handCardIds") ?? [])] as string[];
    let pile = [...(deck.get("drawPile") ?? [])] as string[];
    const actorId = String(pending.get("actorPlayerId") ?? "");
    if (target.get("characterId") === "bart_cassidy" && pile.length)
      hand.push(pile.shift()!);
    if (target.get("characterId") === "el_gringo" && actorId) {
      const actor = await tx.get(room.collection("players").doc(actorId));
      const actorState = await tx.get(
        room.collection("privateStates").doc(actorId),
      );
      const actorHand = [...(actorState.get("handCardIds") ?? [])] as string[];
      if (actorHand.length) {
    ...
    if (
      target.get("characterId") === "suzy_lafayette" &&
      hand.length === 0 &&
      pile.length
    )
      hand.push(pile.shift()!);
...
export const triggerSuzyLafayette = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      !player.exists ||
      player.get("characterId") !== "suzy_lafayette" ||
      player.get("isAlive") === false ||
      hand.length !== 0 ||
      !pile.length
    )
    ...
    tx.update(own.ref, { handCardIds: [drawn] });
...
export const resolveExpiredResponse = onCall(async (request) => {
  ...
  if (!roomId || !pendingActionId)
    throw new HttpsError("invalid-argument", "Thiếu action hết hạn.");
  const room = db.doc(`rooms/${roomId}`);
  await db.runTransaction(async (tx) => {
    const pending = await tx.get(
      room.collection("pendingActions").doc(pendingActionId),
    ...
    const state = await tx.get(room);
    if (!pending.exists || pending.get("status") !== "waiting") return;
    const deadline = pending.get("responseDeadlineAt") as
      admin.firestore.Timestamp | undefined;
    if (!deadline || deadline.toMillis() > Date.now())
      throw new HttpsError("failed-precondition", "Action chưa hết hạn.");
    const targetId = String(
      pending.get("targetPlayerId") ?? pending.get("currentTargetId") ?? "",
    );
