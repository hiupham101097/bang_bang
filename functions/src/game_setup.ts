...
const roles = (count: number): string[] => {
  ...
  if (count < 4 || count > 8)
    throw new HttpsError("failed-precondition", "Phòng chỉ hỗ trợ 4–8 người.");
...
async function initialize(roomId: string) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      ...
      return;
    const players = (await tx.get(roomRef.collection("players"))).docs.sort(
      (a, b) => a.get("seat") - b.get("seat"),
    );
    ...
    if (
      ...
      guests.some(
        (p) => !p.get("isReady") || p.get("connectionState") !== "online",
      )
    )
    ...
    if (offered.length < players.length * 2)
      throw new HttpsError("failed-precondition", "Không đủ thẻ nhân vật.");
    ...
    players.forEach((player, index) => {
      ...
      tx.set(roomRef.collection("privateStates").doc(player.id), {
        ...
        selectedCharacterId: isBot ? options[randomInt(2)] : null,
...
async function finalizeIfReady(roomId: string, force: boolean) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const states = await Promise.all(
      players.map((player) =>
        tx.get(roomRef.collection("privateStates").doc(player.id)),
      ),
    );
    const deadline = room.get(
      "characterDeadlineAt",
    ...
    players.forEach((player, index) => {
      ...
      const options = state.get("characterOptions") as string[];
      const selected =
        state.get("selectedCharacterId") ?? options[randomInt(options.length)];
      const baseHealth =
        characters.find((character) => character[0] === selected)?.[1] ?? 4;
      const health =
        baseHealth + (player.id === room.get("sheriffPlayerId") ? 1 : 0);
    ...
    tx.update(roomRef, {
      ...
      turnDeadlineAt: admin.firestore.Timestamp.fromMillis(
        Date.now() + Number(room.get("turnDurationSeconds")) * 1000,
      ),
      updatedAt: now(),
    });
  });
...
export const beginGameSetup = onDocumentUpdated(
  "rooms/{roomId}",
  async (event) => {
...
export const chooseCharacter = onCall(async (request) => {
  ...
  const { roomId, characterId, actionId } = request.data as {
    roomId?: string;
    characterId?: string;
    actionId?: string;
  };
  if (!roomId || !characterId || !actionId)
    throw new HttpsError("invalid-argument", "Thiếu dữ liệu chọn nhân vật.");
  const roomRef = db.doc(`rooms/${roomId}`);
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (
      ...
      throw new HttpsError("failed-precondition", "Không thể chọn nhân vật.");
    const options = state.get("characterOptions") as string[];
    if (!options.includes(characterId))
      throw new HttpsError(
        "permission-denied",
        "Nhân vật không thuộc lựa chọn của bạn.",
      );
...
export const resumeGameSetup = onCall(async (request) => {
  if (!request.auth)
    throw new HttpsError("unauthenticated", "Yêu cầu đăng nhập.");
  const roomId = String(request.data.roomId ?? "");
  const room = await db.doc(`rooms/${roomId}`).get();
  if (!room.exists) throw new HttpsError("not-found", "Không tìm thấy phòng.");
  if (
    room.get("phase") === "choosing_character" &&
    room.get("characterDeadlineAt").toMillis() <= Date.now()
  )
