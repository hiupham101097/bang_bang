...
const uid = (request) => { if (!request.auth)
...
const member = (id, name, seat, host = false) => ({ playerType: 'human', uid: id, displayName: name, avatarId: 'avatar_01', seat, isHost: host, isReady: false, connectionState: 'online', joinedAt: now() });
...
exports.joinRoom = (0, https_1.onCall)(async (request) => {
    ...
    if (!id && typeof data.roomCode === 'string') {
        const hit = await rooms.where('roomCode', '==', data.roomCode.toUpperCase()).limit(1).get();
        id = hit.docs[0]?.id ?? '';
    }
    if (!id)
    ...
    await db.runTransaction(async (tx) => { const snap = await tx.get(ref); if (!snap.exists || snap.get('status') !== 'waiting' || snap.get('totalPlayerCount') >= snap.get('maxPlayers'))
        throw new https_1.HttpsError('failed-precondition', 'Phòng đã bắt đầu hoặc đầy.'); const mine = await tx.get(ref.collection('players').doc(player)); if (mine.exists)
        return; const players = await tx.get(ref.collection('players')); const occupied = new Set(players.docs.map(d => d.get('seat'))); const seat = Array.from({ length: snap.get('maxPlayers') }, (_, i) => i).find(i => !occupied.has(i)); if (seat === undefined)
        throw new https_1.HttpsError('resource-exhausted', 'Không còn ghế.'); tx.set(ref.collection('players').doc(player), member(player, displayName, seat)); tx.update(ref, { humanPlayerCount: admin.firestore.FieldValue.increment(1), totalPlayerCount: admin.firestore.FieldValue.increment(1), updatedAt: now() }); tx.set(db.doc(`users/${player}`), { currentRoomId: id, updatedAt: now() }, { merge: true }); });
    return { roomId: id };
});
exports.quickJoinRoom = (0, https_1.onCall)(async (request) => { uid(request); const hits = await rooms.where('status', '==', 'waiting').where('isPublic', '==', true).orderBy('updatedAt', 'desc').limit(20).get(); const candidate = hits.docs.filter(d => d.get('totalPlayerCount') < d.get('maxPlayers')).sort((a, b) => (b.get('humanPlayerCount') - a.get('humanPlayerCount')) || (a.get('botPlayerCount') - b.get('botPlayerCount')))[0]; if (!candidate)
    return { roomId: null }; return exports.joinRoom.run({ data: { roomId: candidate.id }, auth: request.auth }); });
exports.setReady = (0, https_1.onCall)(async (request) => { const player = uid(request); const id = roomId(request.data); const ref = rooms.doc(id); await db.runTransaction(async (tx) => { const room = await tx.get(ref); const mine = ref.collection('players').doc(player); if (!room.exists || room.get('status') !== 'waiting' || !(await tx.get(mine)).exists)
    throw new https_1.HttpsError('failed-precondition', 'Không thể sẵn sàng.'); tx.update(mine, { isReady: request.data.isReady === true }); tx.update(ref, { updatedAt: now() }); }); return {}; });
exports.addBot = (0, https_1.onCall)(async (request) => { const owner = uid(request); const id = roomId(request.data); const ref = rooms.doc(id); await db.runTransaction(async (tx) => { const room = await tx.get(ref); if (!room.exists || room.get('hostUid') !== owner || room.get('status') !== 'waiting' || !room.get('allowBots') || room.get('totalPlayerCount') >= room.get('maxPlayers'))
    throw new https_1.HttpsError('permission-denied', 'Không thể thêm bot.'); const players = await tx.get(ref.collection('players')); const occupied = new Set(players.docs.map(d => d.get('seat'))); const seat = Array.from({ length: room.get('maxPlayers') }, (_, i) => i).find(i => !occupied.has(i)); const bot = `bot_${db.collection('_').doc().id}`; tx.set(ref.collection('players').doc(bot), { playerType: 'bot', botId: bot, displayName: 'Billy Bot', avatarId: 'avatar_bot_01', difficulty: request.data.difficulty ?? 'normal', seat, isHost: false, isReady: true, connectionState: 'online', createdAt: now() }); tx.update(ref, { botPlayerCount: admin.firestore.FieldValue.increment(1), totalPlayerCount: admin.firestore.FieldValue.increment(1), updatedAt: now() }); }); return {}; });
exports.removeBot = (0, https_1.onCall)(async (request) => { const owner = uid(request); const id = roomId(request.data); const botId = String(request.data.botId ?? ''); const ref = rooms.doc(id); await db.runTransaction(async (tx) => { const room = await tx.get(ref); const bot = await tx.get(ref.collection('players').doc(botId)); if (!room.exists || room.get('hostUid') !== owner || !bot.exists || bot.get('playerType') !== 'bot')
