...
exports.playSpecialCard = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const roomUpdate = {
            ...
            publicLog: appendPublicLog(state, `${["barrel", "mustang", "appaloosa", "volcanic", "dynamite", "jail"].includes(type) ? "equip" : "play"}:${uid}:${cardId}${targetPlayerId ? `:${targetPlayerId}` : ""}`),
            updatedAt: serverNow(),
        };
...
exports.resolveTurnJudgments = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (dynamite) {
            ...
            const explodes = suitOf(judgement) === "spade" &&
                [
                    "two",
                    "three",
                    "four",
...
exports.resolveTargetCard = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (type === "panico") {
            ...
            const targetIndex = alive.findIndex((player) => player.id === targetPlayerId);
            const baseDistance = Math.min((targetIndex - actorIndex + alive.length) % alive.length, (actorIndex - targetIndex + alive.length) % alive.length);
            ...
            if (distance > 1)
                throw new https_1.HttpsError("failed-precondition", "Mục tiêu ngoài tầm Panico.");
        }
        ...
        tx.update(room, {
            ...
            publicLog: appendPublicLog(state, `play:${uid}:${cardId}:${targetPlayerId}`),
            updatedAt: serverNow(),
        });
    });
    return {};
...
exports.respondDuel = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const hand = [...(privateState.get("handCardIds") ?? [])];
        const validBang = cardId &&
            (cardId === "bang" || cardId.startsWith("bang_")) &&
            hand.includes(cardId);
        if (validBang) {
            const index = hand.indexOf(cardId);
            const nextHand = [...hand.slice(0, index), ...hand.slice(index + 1)];
            const other = action.get("actorPlayerId") === uid
...
exports.useSidKetchum = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        tx.update(privateState.ref, { handCardIds: nextHand });
        tx.update(player.ref, {
            health: Math.min(Number(player.get("maxHealth")), Number(player.get("health")) + 1),
            cardCount: nextHand.length,
        });
        tx.update(deck.ref, { discardPile: discard });
        tx.update(room, {
...
exports.openKitCarlson = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!state.exists ||
            ...
            action.exists ||
            pile.length < 3)
            throw new https_1.HttpsError("failed-precondition", "Không thể mở kỹ năng Kit Carlson.");
...
exports.chooseKitCarlson = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const own = await tx.get(room.collection("privateStates").doc(uid));
        const deck = await tx.get(room.collection("serverState").doc("deck"));
        const choices = [...(action.get("choices") ?? [])];
        if (!state.exists ||
            state.get("phase") !== "character_skill_selection" ||
            !action.exists ||
            action.get("actorPlayerId") !== uid ||
            !cardIds.every((id) => choices.includes(id)))
            throw new https_1.HttpsError("failed-precondition", "Lựa chọn Kit Carlson không hợp lệ.");
...
exports.openLuckyDukeJudgment = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!player.exists ||
            ...
            action.exists ||
            pile.length < 2)
            throw new https_1.HttpsError("failed-precondition", "Không thể dùng Lucky Duke.");
...
exports.chooseLuckyDukeJudgment = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!action.exists ||
            ...
            action.get("actorPlayerId") !== uid)
            throw new https_1.HttpsError("failed-precondition", "Phán xét Lucky Duke không hợp lệ.");
        const choices = [...(action.get("choices") ?? [])];
        if (!choices.includes(resultCardId))
            throw new https_1.HttpsError("invalid-argument", "Kết quả không thuộc hai lá đã rút.");
        result = resultCardId;
        tx.update(deck.ref, {
...
exports.resolveJourdonnais = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!state.exists ||
            ...
            throw new https_1.HttpsError("failed-precondition", "Không thể phán xét Jourdonnais.");
        const pile = [...(deck.get("drawPile") ?? [])];
        if (!pile.length)
            throw new https_1.HttpsError("failed-precondition", "Hết bài phán xét.");
...
exports.useCalamityJanetDodge = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!state.exists ||
            ...
            Number(pending.get("requiredDodges") ?? 1) > 1 ||
            !hand.includes(cardId))
            throw new https_1.HttpsError("failed-precondition", "Không thể dùng kỹ năng Calamity Janet.");
...
exports.resolveSlabDodge = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const own = await tx.get(room.collection("privateStates").doc(uid));
        const deck = await tx.get(room.collection("serverState").doc("deck"));
        const attacker = await tx.get(room
            .collection("players")
            .doc(String(pending.get("actorPlayerId") ?? "")));
        const hand = [...(own.get("handCardIds") ?? [])];
        if (!pending.exists ||
            pending.get("actionType") !== "bang" ||
            pending.get("targetPlayerId") !== uid ||
            !attacker.exists ||
            ...
            throw new https_1.HttpsError("failed-precondition", "Không thể né Bang của Slab.");
        const nextHand = hand.filter((card) => !cardIds.includes(card));
        const discard = [...(deck.get("discardPile") ?? []), ...cardIds];
...
exports.acceptBangDamage = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!pending.exists ||
            ...
            throw new https_1.HttpsError("failed-precondition", "Không có Bang cần nhận sát thương.");
        let hand = [...(own.get("handCardIds") ?? [])];
        let pile = [...(deck.get("drawPile") ?? [])];
        ...
        if (target.get("characterId") === "suzy_lafayette" &&
            hand.length === 0 &&
            pile.length)
            hand.push(pile.shift());
...
exports.resolveExpiredResponse = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        const pending = await tx.get(room.collection("pendingActions").doc(pendingActionId));
        const state = await tx.get(room);
        if (!pending.exists || pending.get("status") !== "waiting")
            return;
        const deadline = pending.get("responseDeadlineAt");
        if (!deadline || deadline.toMillis() > Date.now())
            throw new https_1.HttpsError("failed-precondition", "Action chưa hết hạn.");
        const targetId = String(pending.get("targetPlayerId") ?? pending.get("currentTargetId") ?? "");
