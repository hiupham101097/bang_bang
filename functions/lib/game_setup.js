...
const shuffled = (values) => { const copy = [...values]; for (let i = copy.length - 1; i > 0; i--) {
...
async function initialize(roomId) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!room.exists || room.get('status') !== 'starting' || room.get('phase') !== 'assigning_roles')
            return;
        ...
        if (players.length < 4 || players.length > 8 || humans.length < 2 || players.length > room.get('maxPlayers') || new Set(players.map(p => p.id)).size !== players.length || new Set(players.map(p => p.get('seat'))).size !== players.length || humans.some(p => !p.get('isReady')))
        ...
        players.forEach((player, index) => {
            ...
            tx.set(roomRef.collection('privateStates').doc(player.id), { role, roleAssigned: true, characterOptions: options, selectedCharacterId: isBot ? options[(0, crypto_1.randomInt)(2)] : null, characterSelectionSubmitted: isBot, handCardIds: [] });
        });
...
async function finalizeIfReady(roomId, force) {
    ...
    await db.runTransaction(async (tx) => {
        const room = await tx.get(roomRef);
        if (!room.exists || room.get('status') !== 'starting' || room.get('phase') !== 'choosing_character')
            return;
        const players = (await tx.get(roomRef.collection('players'))).docs.sort((a, b) => a.get('seat') - b.get('seat'));
        ...
        if (!force && states.some(state => !state.get('characterSelectionSubmitted')) && deadline.toMillis() > Date.now())
            return;
...
exports.beginGameSetup = (0, firestore_1.onDocumentUpdated)('rooms/{roomId}', async (event) => {
    ...
    if (before?.status !== 'starting' && after?.status === 'starting' && after?.phase === 'assigning_roles')
...
exports.chooseCharacter = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (tx) => { const room = await tx.get(roomRef); const state = await tx.get(stateRef); if (!room.exists || room.get('phase') !== 'choosing_character' || !state.exists || state.get('characterSelectionSubmitted') || room.get('characterDeadlineAt').toMillis() < Date.now())
        throw new https_1.HttpsError('failed-precondition', 'Không thể chọn nhân vật.'); const options = state.get('characterOptions'); if (!options.includes(characterId))
        throw new https_1.HttpsError('permission-denied', 'Nhân vật không thuộc lựa chọn của bạn.'); const action = await tx.get(roomRef.collection('actions').doc(actionId)); if (action.exists)
        return; tx.set(action.ref, { uid: playerId, type: 'choose_character', createdAt: now() }); tx.update(stateRef, { selectedCharacterId: characterId, characterSelectionSubmitted: true }); });
    await finalizeIfReady(roomId, false);
    return { roomId };
});
exports.resumeGameSetup = (0, https_1.onCall)(async (request) => { if (!request.auth)
    throw new https_1.HttpsError('unauthenticated', 'Yêu cầu đăng nhập.'); const roomId = String(request.data.roomId ?? ''); const room = await db.doc(`rooms/${roomId}`).get(); if (!room.exists)
    throw new https_1.HttpsError('not-found', 'Không tìm thấy phòng.'); if (room.get('phase') === 'choosing_character' && room.get('characterDeadlineAt').toMillis() <= Date.now())
    await finalizeIfReady(roomId, true); return { status: room.get('status'), phase: room.get('phase'), sheriffPlayerId: room.get('sheriffPlayerId') }; });
